...
void clear_screen() { terminal_initialize(); }
...
void terminal_putchar(char ch)
{
    ...
    if (ch) terminal_putentryat(ch, terminal_color, terminal_row, terminal_col);
...
{
    size_t i = 0;
    for (; i < len; i++) terminal_putchar(str[i]);
}

void terminal_writestring(const char *str_ptr)
{
    terminal_write(str_ptr, str_len(str_ptr));
}
